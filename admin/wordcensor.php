<?php
  ...
  $noRightNav = '';
  $headerTitle = 'Actions:';
  $pageTitle = 'ARIES - Word Censor Editor';
  $mainContent = $content;
  ...
  $Censor_Main = $template->getSection('WordCensorMain');
  $Censor_Main = str_replace('[Censor_Left]', $mainContent, $Censor_Main);
  $Censor_Main = str_replace('[Censor_Right]', $rightNav, $Censor_Main);
  $mainContent= $Censor_Main;  
  if (!$msg==""){
	  $msg='
  ...
  function insertWordCensor()
  {
    ...
    {
      $msg = ' You must enter a spelling mistake and the replace_with.' . "\n";
    }
    else
    {
      ...
      {
        $msg = ' There was a problem editing the replace_with - no changes made.';
      }
    }
    return $msg;
  }
