...
***************************************/
  ...
  $convo = (isset ($get_vars['id'])) ? getuserConvo($get_vars['id'], $show) : "<br>Please select a conversation from the side bar.";
  ...
  $Log_Main=$template->getSection('ConversationLogsMain');
  $Log_Main = str_replace('[Log_Left]', $mainContent, $Log_Main);
  $Log_Main = str_replace('[Log_Right]', $rightNav, $Log_Main);
  $mainContent= $Log_Main;  
	if (!$msg==""){
	  $msg='
...
$options
        ...
        <input type="submit" id="submit" name="submit" value="show" class="btn btn-primary">
      </form>
	  <br />
endForm;
      return $form;
  ...
  */
  function getuserConvo($id, $showing)
  {
    ...
    $list = "<h4>$title conversations for user: $id</h4>";
    $list .= "<div class=\"well\">";
    $result = db_fetchAll($sql, null, __FILE__, __FUNCTION__, __LINE__);
    foreach ($result as $row)
    {
      ...
      if ($thisdate != $lasttimestamp)
      {
        ...
        $date = date("Y-m-d");
        $list .= "<hr><h5>Conversation#$i $thisdate</h5>";
        $i++;
      }
      $list .= "<br><span style=\"color:DARKBLUE;\" class=\"fa fa-user\"> $user_name: " . $row['input'] . "</span>";
      $list .= "<br><span style=\"color:GREEN;\" class=\"fa fa-android\"> $bot_name: " . $row['response'] . "</span>";
